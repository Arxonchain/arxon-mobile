...
async function generateVapidAuthHeader(
  ...
  const payload = {
    ...
    sub: "mailto:gabemetax@gmail.com",
  };
      ...
      body: JSON.stringify({
        message: {
          ...
          android: {
            ...
            notification: {
              sound: "default",
              channel_id: "arxon_default",
              // Must match an Android resource name in the app (no file extension)
              icon: "ic_launcher",
            },
          },
        },
    ...
    const results = await Promise.all(
      subscriptions.map(async (sub: any) => {
        ...
        if (isFCM) {
          ...
          const ok = await sendFCMv1(token, title || "ARXON", body || "New notification", dataStrings, firebaseProject, serviceAccount);
