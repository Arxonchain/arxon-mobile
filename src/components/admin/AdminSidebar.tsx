...
const SidebarContent = ({ collapsed, onCollapse, onNavigate }: {
  collapsed: boolean; onCollapse?: () => void; onNavigate?: () => void;
}) => {
    ...
    <>
      ...
      <nav className="space-y-1 px-3 flex-1">
        {navItems.map((item) =>
          item.external ? (
            ...
            <NavLink key={item.path} to={item.path} end={item.path === "/admin"} onClick={onNavigate}
      ...
      <div className="px-3 space-y-1 mt-auto border-t border-border/50 pt-4">
        <button onClick={() => navigate("/")}
          className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg w-full transition-colors mb-1",
        ...
        <button onClick={handleLogout}
          className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg w-full transition-colors",
