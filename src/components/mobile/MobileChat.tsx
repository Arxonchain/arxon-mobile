...
function Av({url,name,col,sz=28}:{url:string|null;name:string|null;col:string;sz?:number}) {
  ...
  if (url&&!err) return (
    <div style={{width:sz,height:sz,borderRadius:sz*.35,overflow:'hidden',flexShrink:0,border:`1px solid ${col}22`}}>
      <img src={url} alt="" onError={()=>setErr(true)} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
    </div>
  );
  return (
    <div style={{width:sz,height:sz,borderRadius:sz*.35,flexShrink:0,fontSize:sz*.4,fontWeight:700,
      background:`${col}18`,border:`1px solid ${col}33`,color:col,
...
export default function MobileChat() {
  ...
  // Close menu on scroll/tap outside
  useEffect(()=>{
    if (!menu) return;
  ...
  // Bulletproof long press — works on both mobile touch and desktop mouse
  ...
  return (
    <div style={{minHeight:'100vh',background:'hsl(225 30% 3%)',display:'flex',
      ...
      paddingBottom:90}} onClick={()=>setMenu(null)}>
      ...
      {menu && (
        <div style={{position:'fixed',zIndex:9999,
          ...
          left: Math.min(Math.max(menu.x-110,10), window.innerWidth-230),
          background:'hsl(225 26% 11%)',border:'1px solid hsl(215 22% 20%)',
          borderRadius:18,overflow:'hidden',minWidth:220,
          boxShadow:'0 8px 40px rgba(0,0,0,0.6)'}}
          onClick={e=>e.stopPropagation()}>
          {/* Preview */}
          <div style={{padding:'12px 14px',borderBottom:'1px solid hsl(215 22% 15%)',
            background:'hsl(215 25% 9%)'}}>
            <p style={{fontSize:10,fontWeight:700,color:col,marginBottom:4}}>
              {menu.msg.username||'Miner'}
            </p>
            <p style={{fontSize:12,color:'hsl(215 18% 65%)',lineHeight:1.4,
          ...
          {/* Reply */}
          <button onTouchEnd={e=>{e.preventDefault();doReply(menu.msg);}}
          ...
          {/* Copy */}
          <button onTouchEnd={e=>{e.preventDefault();doCopy(menu.msg);}}
            onClick={()=>doCopy(menu.msg)}
            style={{width:'100%',display:'flex',alignItems:'center',gap:12,
              padding:'13px 16px',background:'none',border:'none',cursor:'pointer',
              outline:'none',borderBottom: menu.msg.user_id===user?.id?'1px solid hsl(215 22% 13%)':'none'}}>
      ...
      <div className="scrollbar-none"
        ...
        {CHANNELS.map(c=>{
          ...
          return (
            <button key={c.id} onClick={()=>ac&&setCh(c.id)}
              style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',
                borderRadius:22,fontSize:12,fontWeight:700,border:'none',
                cursor:ac?'pointer':'not-allowed',outline:'none',transition:'all .2s',flexShrink:0,
                background:ia?`${CC[c.id]}22`:'hsl(215 22% 10%)',
                color:ia?CC[c.id]:ac?'hsl(215 18% 45%)':'hsl(215 14% 28%)',
                borderWidth:1,borderStyle:'solid',
                borderColor:ia?`${CC[c.id]}44`:'hsl(215 20% 16%)',opacity:ac?1:.45}}>
      ...
      <div style={{padding:'7px 20px 0',flexShrink:0}}>
        <p style={{fontSize:11,color:'hsl(215 14% 35%)'}}>
          ...
          {' '}· <span style={{color:'hsl(215 14% 28%)'}}>Hold message to reply/copy/delete</span>
        </p>
      </div>

      {ok===false&&(
        <div style={{margin:'10px 20px',padding:'13px 15px',borderRadius:14,
          background:'hsl(38 55% 52%/0.08)',border:'1px solid hsl(38 55% 52%/0.22)',flexShrink:0}}>
          <p style={{fontSize:12,fontWeight:600,color:'hsl(38 55% 58%)',marginBottom:3}}>
            ⚙️ Chat Setup Required
          </p>
          <p style={{fontSize:11,color:'hsl(38 55% 52%/0.7)',lineHeight:1.5}}>
            Run the SQL migration in Supabase to enable chat.
          </p>
        </div>
      )}
      ...
      {/* Messages */}
      <div className="scrollbar-none"
        style={{flex:1,overflowY:'auto',padding:'10px 20px 0',minHeight:0}}>
        {load&&ok===null&&(
        ...
        {ok===true&&msgs.map((m,i)=>{
          ...
          return (
            <div key={m.id}
              ...
              style={{marginBottom:sh?12:4,display:'flex',
              ...
              <div style={{maxWidth:'76%'}}>
                ...
                {m.reply_to_id&&m.reply_to_message&&(
                  <div style={{marginBottom:2,padding:'7px 11px',
                    borderRadius:'12px 12px 0 0',
                    background:'hsl(215 22% 13%)',
                    borderLeft:`3px solid ${col}`}}>
                    <p style={{fontSize:10,fontWeight:700,color:col,marginBottom:2}}>
                      {m.reply_to_username||'Miner'}
                    </p>
                    <p style={{fontSize:11,color:'hsl(215 14% 48%)',
                ...
                <div style={{padding:'10px 14px',
                  ...
                  background: me
                    ?`linear-gradient(135deg,${col}22,${col}10)`
                    :'hsl(225 24% 10%)',
                  border:`1px solid ${me?`${col}30`:'hsl(215 22% 16%)'}`,
                  wordBreak:'break-word'}}>
                  <p style={{fontSize:13,color:'hsl(215 18% 88%)',lineHeight:1.55,margin:0}}>
                    {m.message}
                  </p>
                </div>
      ...
      {rep&&(
        <div style={{margin:'0 20px 6px',padding:'9px 12px',borderRadius:14,
          background:'hsl(215 22% 10%)',border:`1px solid ${col}35`,
          flexShrink:0,display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:3,alignSelf:'stretch',minHeight:32,borderRadius:2,
            background:col,flexShrink:0}}/>
          <div style={{flex:1,minWidth:0}}>
            <p style={{fontSize:11,fontWeight:700,color:col,marginBottom:2}}>
              ↩ Replying to {rep.username||'Miner'}
            </p>
            <p style={{fontSize:11,color:'hsl(215 14% 45%)',
              overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
              {rep.message}
            </p>
          </div>
          <button onClick={()=>setRep(null)}
          ...
          {!can(ch)?(
            <div style={{padding:'14px 16px',borderRadius:18,textAlign:'center',
              ...
              <p style={{fontSize:12,color:'hsl(215 18% 40%)'}}>
                🔒 Join {ch==='alpha'?'Alpha':'Omega'} team in Arena
              </p>
            </div>
          ):(
        ...
        <div style={{padding:'12px 20px',flexShrink:0}}>
          <button onClick={()=>navigate('/auth')}
            style={{width:'100%',padding:'16px',borderRadius:18,fontWeight:700,fontSize:14,
              cursor:'pointer',
              background:'linear-gradient(135deg,hsl(215 35% 55%),hsl(215 40% 45%))',
