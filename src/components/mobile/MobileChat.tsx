  ...
  const loadMessages = useCallback(async (ch: Channel) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '42P01') { setTableOk(false); setMessages([]); }
        else throw error;
      } else {
        setTableOk(true);
          ...
          return (
            <button key={ch.id}
              ...
              style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',
                borderRadius:22,fontSize:12,fontWeight:700,border:'none',cursor:accessible?'pointer':'not-allowed',
      ...
      {!tableOk && (
        <div style={{margin:'12px 20px',padding:'14px 16px',borderRadius:16,
          ...
          <p style={{fontSize:11,color:'hsl(38 55% 52%/0.7)',lineHeight:1.5}}>
            Run the SQL migration in your Supabase dashboard to enable chat. See the README for instructions.
          </p>
        </div>
      )}
      ...
      <div className="scrollbar-none"
        ...
        {loading ? (
          <div style={{display:'flex',justifyContent:'center',paddingTop:40}}>
            <div style={{width:32,height:32,borderRadius:'50%',border:'2px solid hsl(215 35% 62%/0.2)',
            ...
            <div key={msg.id} style={{marginBottom:showHeader?12:4,display:'flex',
              ...
              <div style={{maxWidth:'72%'}}>
                ...
                <div style={{padding:'10px 14px',borderRadius:isMe?'18px 18px 4px 18px':'18px 18px 18px 4px',
                  background:isMe
                    ?`linear-gradient(135deg,${chanColor}22,${chanColor}12)`
                    :'hsl(225 24% 10%)',
                  border:`1px solid ${isMe?`${chanColor}30`:'hsl(215 22% 16%)'}`,
                  wordBreak:'break-word'}}>
                  <p style={{fontSize:13,color:'hsl(215 18% 88%)',lineHeight:1.55}}>{msg.message}</p>
                </div>
                {isMe && (
                  <p style={{fontSize:9,color:'hsl(215 14% 28%)',marginTop:3,textAlign:'right'}}>{fmtTime(msg.created_at)}</p>
                )}
              </div>
            </div>
          );
        ...
        <div style={{padding:'12px 20px',flexShrink:0,
          ...
          {!canAccess(channel) ? (
            <div style={{padding:'14px 16px',borderRadius:18,textAlign:'center',
              ...
              <p style={{fontSize:12,color:'hsl(215 18% 40%)'}}>🔒 Join {channel === 'alpha' ? 'Alpha' : 'Omega'} team in Arena to access this channel</p>
            </div>
          ) : (
            <div style={{display:'flex',alignItems:'flex-end',gap:10,
              ...
              <button onClick={sendMsg} disabled={!input.trim()||sending}
                style={{width:38,height:38,borderRadius:13,flexShrink:0,cursor:input.trim()?'pointer':'default',
                  background:input.trim()?`${chanColor}22`:'hsl(215 22% 12%)',
                  border:`1px solid ${input.trim()?`${chanColor}44`:'hsl(215 22% 18%)'}`,
                  display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.2s',outline:'none'}}>
      ...
      {!user && (
        <div style={{padding:'12px 20px',flexShrink:0}}>
          <button onClick={()=>navigate('/auth')}
            style={{width:'100%',padding:'16px',borderRadius:18,fontWeight:700,fontSize:14,cursor:'pointer',
