...
@keyframes mineGlow{0%,100%{box-shadow:0 4px 24px rgba(139,174,214,.3),0 0 0 1px rgba(139,174,214,.2)}50%{box-shadow:0 4px 32px rgba(139,174,214,.6),0 0 0 1px rgba(139,174,214,.4)}}
`;
  ...
  return (
    <div style={{animation:'coinfloat3d 4s ease-in-out infinite',position:'relative',width:92,height:92}}>
      <svg width="92" height="92" viewBox="0 0 92 92">
        <defs>
          <radialGradient id="dcf2" cx="36%" cy="30%" r="68%"><stop offset="0%" stopColor="#C8E0FF"/><stop offset="30%" stopColor="#8BAED6"/><stop offset="65%" stopColor="#3A6898"/><stop offset="100%" stopColor="#0E2244"/></radialGradient>
          <linearGradient id="dce2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#4A7AB8"/><stop offset="50%" stopColor="#1E4A80"/><stop offset="100%" stopColor="#081828"/></linearGradient>
          <radialGradient id="dch2" cx="35%" cy="25%" r="55%"><stop offset="0%" stopColor="rgba(255,255,255,0.3)"/><stop offset="100%" stopColor="rgba(255,255,255,0)"/></radialGradient>
          <linearGradient id="dcr2" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="rgba(139,174,214,0.6)"/><stop offset="100%" stopColor="rgba(30,74,128,0.3)"/></linearGradient>
          <filter id="dcs2"><feDropShadow dx="0" dy="4" stdDeviation="6" floodColor="rgba(0,0,0,0.5)"/></filter>
        </defs>
        <ellipse cx="46" cy="49" rx="8" ry="3" fill="url(#dce2)"/>
        <ellipse cx="46" cy="47" rx="36" ry="9" fill="url(#dce2)"/>
        <circle cx="46" cy="46" r="36" fill="url(#dcf2)" filter="url(#dcs2)"/>
        <circle cx="46" cy="46" r="34" fill="none" stroke="rgba(255,255,255,.06)" strokeWidth="1"/>
        <circle cx="46" cy="46" r="38" fill="none" stroke="url(#dcr2)" strokeWidth="3.5"/>
        <ellipse cx="38" cy="30" rx="22" ry="12" fill="url(#dch2)" style={{transform:'rotate(-25deg)',transformOrigin:'38px 30px'}}/>
...
export default function MobileDashboard() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#000',fontFamily:"'Creato Display',-apple-system,system-ui,sans-serif",paddingBottom:90}}>
      ...
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.45}} style={{margin:'8px 16px 0'}}>
        <HeroCard>
          <div style={{padding:'18px 18px 16px',display:'flex',gap:0,alignItems:'stretch'}}>
            ...
            <div style={{width:108,flexShrink:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}><ArxonCoin/></div>
          </div>
        </HeroCard>
      </motion.div>
      ...
      {/* Quick Access — Arena | Mine (center big) | Nexus */}
      <div style={{padding:'16px 20px 6px',fontSize:10,textTransform:'uppercase',letterSpacing:'1.2px',color:'rgba(238,242,247,.22)',fontWeight:600}}>Quick Access</div>
      <div style={{display:'flex',alignItems:'flex-end',gap:12,padding:'0 16px 16px'}}>

        {/* Arena */}
        <motion.button whileTap={{scale:.9}} onClick={()=>navigate('/arena')}
          style={{flex:1,background:'linear-gradient(145deg,#0c1e38,#061220)',border:'1.5px solid rgba(139,174,214,0.35)',borderRadius:22,padding:'16px 8px 12px',display:'flex',flexDirection:'column',alignItems:'center',gap:8,cursor:'pointer',position:'relative',overflow:'hidden'}}>
          <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:.4}} viewBox="0 0 120 100">
            <path d="M 5 2 Q 60 -4 115 2" fill="none" stroke="rgba(180,95,255,0.5)" strokeWidth="1.5" strokeDasharray="300" style={{animation:'strokeDash 3s ease-in-out infinite alternate'}}/>
          </svg>
          <div style={{width:44,height:44,borderRadius:14,background:'rgba(180,95,255,.12)',border:'1px solid rgba(180,95,255,.25)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',zIndex:1}}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#B45FFF" strokeWidth="2"><path d="M14.5 17.5L3 6V3h3l11.5 11.5"/><circle cx="19" cy="19" r="2"/><circle cx="5" cy="5" r="2"/></svg>
          </div>
          <span style={{fontSize:12,fontWeight:700,color:'rgba(200,228,255,.8)',position:'relative',zIndex:1}}>Arena</span>
        </motion.button>

        {/* Mine — center, bigger, stunning */}
        <motion.button whileTap={{scale:.88}} onClick={()=>navigate('/mining')}
          style={{flex:0,width:90,background:'linear-gradient(145deg,#1E3A5F,#0c2040,#061220)',border:'2px solid rgba(139,174,214,.45)',borderRadius:26,padding:'20px 8px 14px',display:'flex',flexDirection:'column',alignItems:'center',gap:8,cursor:'pointer',position:'relative',overflow:'hidden',marginBottom:8,animation:'mineGlow 2.5s ease-in-out infinite',boxShadow:'0 4px 28px rgba(30,58,95,.6)'}}>
          {/* Stroke ring on mine button */}
          <svg style={{position:'absolute',inset:0,width:'100%',height:'100%'}} viewBox="0 0 90 120" preserveAspectRatio="none">
            <path d="M 6 3 Q 45 -3 84 3" fill="none" stroke="rgba(139,174,214,0.6)" strokeWidth="2" strokeDasharray="200" style={{animation:'strokeDash 2s ease-in-out infinite alternate'}}/>
            <path d="M 6 117 Q 45 123 84 117" fill="none" stroke="rgba(168,196,232,0.4)" strokeWidth="2" strokeDasharray="200" style={{animation:'strokeDash 2s ease-in-out infinite alternate',animationDelay:'.5s'}}/>
          </svg>
          <div style={{position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 0%,rgba(139,174,214,.08) 0%,transparent 70%)',pointerEvents:'none'}}/>
          <div style={{width:48,height:48,borderRadius:16,background:'linear-gradient(135deg,rgba(139,174,214,.2),rgba(139,174,214,.08))',border:'1px solid rgba(139,174,214,.35)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',zIndex:1,boxShadow:'0 2px 16px rgba(139,174,214,.2)'}}>
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#A8C4E8" strokeWidth="2.2" strokeLinecap="round"><path d="M15 5l4 4-8 8-5 1 1-5 8-8z"/><path d="M12 8l4 4"/></svg>
          </div>
          <span style={{fontSize:13,fontWeight:800,color:'#C8E0FF',letterSpacing:'.02em',position:'relative',zIndex:1}}>MINE</span>
          {isMining && <div style={{position:'absolute',bottom:8,width:6,height:6,borderRadius:'50%',background:'#5DB08A',boxShadow:'0 0 8px #5DB08A',animation:'pulse 2s infinite'}}/>}
        </motion.button>

        {/* Nexus */}
        <motion.button whileTap={{scale:.9}} onClick={()=>navigate('/nexus')}
          style={{flex:1,background:'linear-gradient(145deg,#0c1e38,#061220)',border:'1.5px solid rgba(139,174,214,0.35)',borderRadius:22,padding:'16px 8px 12px',display:'flex',flexDirection:'column',alignItems:'center',gap:8,cursor:'pointer',position:'relative',overflow:'hidden'}}>
          <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:.4}} viewBox="0 0 120 100">
            <path d="M 5 2 Q 60 -4 115 2" fill="none" stroke="rgba(93,176,138,0.5)" strokeWidth="1.5" strokeDasharray="300" style={{animation:'strokeDash 3s ease-in-out infinite alternate',animationDelay:'.3s'}}/>
          </svg>
          <div style={{width:44,height:44,borderRadius:14,background:'rgba(93,176,138,.1)',border:'1px solid rgba(93,176,138,.22)',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',zIndex:1}}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#5DB08A" strokeWidth="1.8"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
          </div>
          <span style={{fontSize:12,fontWeight:700,color:'rgba(200,228,255,.8)',position:'relative',zIndex:1}}>Nexus</span>
        </motion.button>
      </div>
      ...
      <div style={{display:'flex',gap:10,padding:'0 16px 14px',overflowX:'auto',scrollbarWidth:'none'}}>
        <motion.div whileTap={{scale:.97}} onClick={()=>navigate('/arena')} style={{minWidth:200,borderRadius:20,overflow:'hidden',position:'relative',height:130,flexShrink:0,cursor:'pointer'}}>
          ...
          <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',opacity:.08}} viewBox="0 0 200 130"><path d="M0 40h40v-20h120v20h40" stroke="#8BAED6" strokeWidth=".8" fill="none"/><circle cx="80" cy="40" r="3" fill="#8BAED6"/><circle cx="120" cy="65" r="3" fill="#8BAED6"/></svg>
